...
private:
	...
	//bool __vecVisitBool[10000];
	//int __vecVisitNode[10000];
	bool* __vecVisitBool;
	int* __vecVisitNode;
...
public:
	...
	void build_n_RRsets(const long long numSamples)
	{
		if (numSamples > 2147483647)
		{
		...
		//std::cout << "sum tt time "<<sumTT << std::endl;
	}
	...
	double eval_inf_spread(const std::vector<int>& vecSeed)
	{
		...
		int c = 0;
		for(int i=0;i<vecBoolVst.size();i++){
			if(vecBoolVst[i] == true){
				c++;			
			}		
		}
		inf = (double)c * __numV / __numRRsets;
		return inf;
	}
